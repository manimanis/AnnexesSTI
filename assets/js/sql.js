...
createApp({
  ...
  methods: {
    ...
    formatCodeSnippet(code) {
      ...
      const sqlRegex = /(--.*|\/\*[\s\S]*?\*\/)|(".*?"|'.*?')|(\b(?:COUNT|SUM|AVG|MAX|MIN|NOW|CURRENT_DATE|YEAR|MONTH|DAY|CONCAT|SUBSTRING|UPPER|LOWER|LENGTH)\b)|(\b(?:SELECT|DISTINCT|FROM|WHERE|INSERT|INTO|UPDATE|SET|DELETE|CREATE|DATABASE|USE|TABLE|ALTER|ADD|MODIFY|CHANGE|DROP|COLUMN|PRIMARY|KEY|FOREIGN|REFERENCES|JOIN|INNER|ON|GROUP|BY|HAVING|ORDER|ASC|DESC|AND|OR|NOT|IN|LIKE|BETWEEN|NULL|IS|INT|INTEGER|VARCHAR|CHAR|TEXT|DECIMAL|DATE|DATETIME|TIMESTAMP|AUTO_INCREMENT|DEFAULT|CHECK|UNIQUE|CASCADE|RESTRICT|SOURCE)\b)|(\b\d+\b)/gi;
