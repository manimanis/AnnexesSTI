...
createApp({
  ...
  methods: {
    ...
    copy(text) {
      navigator.clipboard.writeText(text).then(() => alert('Code SQL copié !'));
    },
    highlight(text) {
    ...
    formatCodeSnippet(code) {
      ...
      const tokenRegex = /(--[^\n]*)|('.*?')|\b(SELECT|FROM|WHERE|INSERT|INTO|VALUES|UPDATE|SET|DELETE|CREATE|TABLE|ALTER|DROP|PRIMARY|KEY|FOREIGN|REFERENCES|NOT|NULL|UNIQUE|CHECK|DEFAULT|ORDER|BY|GROUP|BY|HAVING|ASC|DESC|AND|OR|BETWEEN|LIKE|IN|IS|AUTO_INCREMENT|CASCADE|DECIMAL|INT|INTEGER|VARCHAR|CHAR|DATE)\b|\b(COUNT|AVG|SUM|MIN|MAX|NOW|CURRENT_DATE|YEAR|MONTH|DAY)\b|\b(\d+)\b/g;

      return text.replace(tokenRegex, (match, comment, str, kw, fn, num) => {
