/* =============================================
   Annexe PHP - Script Vue.js externe
   ============================================= */
const categoryOrder = [
...
createApp({
  setup() {
    ...
    const selectedTocItemId = ref(localStorage.getItem('php_last_selected_item') || null);
    const items = ref([]);
    ...
    onMounted(async () => {
      try {
        ...
        console.error('Erreur lors du chargement de jsons/php.json:', e);
      }
    ...
    watch(selectedTocItemId, (newId) => {
      if (newId) {
        localStorage.setItem('php_last_selected_item', newId);
      } else {
        localStorage.removeItem('php_last_selected_item');
      }
    });
    ...
    const copy = (text) => {
      navigator.clipboard.writeText(text).then(() => alert('Code PHP copié !'));
    };
